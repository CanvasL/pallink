...
- `swagger-ui` 容器会根据 `SWAGGER_API_HOST`、`SWAGGER_API_BASE_PATH`、`SWAGGER_API_SCHEMES` 在启动时渲染最终的 `swagger.json`，不需要把 API 域名硬编码进仓库里的 Swagger 文件
...
- [`gateway/swagger.json`](./gateway/swagger.json)
